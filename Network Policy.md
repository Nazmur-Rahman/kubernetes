...
* All Pods can talk to all other Pods in the cluster (full connectivity).
* There are no restrictions (like a big open LAN).
* A NetworkPolicy acts like a *firewall* for Pods — it lets you define rules for allowing or blocking traffic based on:
  * Pod labels
  * Namespaces
  * IP blocks
  * Ports and protocols
...
**🔑 Key Concepts**
  ...
  * *Selectors →* Decide which Pods the policy applies to (via labels).
  * *Default behavior →* If a Pod has no NetworkPolicy, it accepts all traffic. Once you attach a policy, traffic is denied by default unless explicitly allowed.
